...
const ContactManager = () => {
  const [contacts, setContacts] = useState([
    { id: '1', name: 'Aneesh', surname: 'Rao', mobile: '888/143029', favorite: false },
    { id: '2', name: 'Jatin', surname: 'Rana', mobile: '874/985032', favorite: false }
  ]);
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-br from-green-600 to-purple-800 p-6">
      {/* Left Sidebar */}
      <div className="w-80 bg-white/10 backdrop-blur-lg rounded-2xl p-6 mr-6 shadow-xl">
        <div className="flex items-center space-x-3 text-white mb-8">
          ...
          <span className="text-lg font-semibold">Mayur Singh Shekhawat</span>
        </div>

        <div className="space-y-4">
          <button 
            className={`w-full ${!showFavorites ? 'bg-gray-500' : 'bg-purple-800/50'} text-white p-3 rounded-xl flex items-center space-x-3 transition-all hover:bg-purple-500`}
            onClick={() => setShowFavorites(false)}
          >
            <User className="w-5 h-5" />
            <span>All contacts</span>
            <span className="ml-auto bg-gray-700 px-3 py-1 rounded-lg">
              {contacts.length}
            </span>
          </button>
          ...
          <form onSubmit={handleAdd} className="space-y-3 mt-8">
            ...
            <button 
              type="submit"
              className="w-full bg-blue-500 text-white p-3 rounded-xl hover:bg-purple-400 transition-colors font-semibold"
            >
              Add Contact
            </button>
          </form>
